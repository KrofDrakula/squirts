...
World.prototype.getPlayerBlob = function() {
    var results = this.blobs.filter(function(b) {
        return b.isPlayer;
    });
...
World.prototype.handleMouseDown = function(ev) {
    ...
    this.getPlayerBlob().squirt(real);
};
...
World.prototype.generate = function(options) {
    ...
    for (var i = 0; i < options.count; i++) {
        this.addBlob(new Blob({
            ...
            radius   : !playerGenerated ? options.radius * 0.5 : options.radius * 0.1 + Math.random() * options.radius * 0.9,
            speed    : new Vector2d(0, 0),
            isPlayer : !playerGenerated
        }));
        playerGenerated = true;
    }
};

World.prototype.clear = function(reset) {
    var ctx = this.canvas.getContext('2d');
    ctx.fillStyle = reset ? 'black' : 'rgba(0,0,0,0.1)';
...
World.prototype.draw = function() {
    ...
    for (var i = 0; i < this.blobs.length; i++) {
        ctx.fillStyle = this.blobs[i].isPlayer ? 'blue' : 'red';
