...
World.prototype.handleBlobSlurped = function(eater, eaten) {
    var self      = this,
        position  = eaten.position.sub(eater.position).normalize().scale(eater.radius).add(eater.position),
    ...
    emitter.on('dead', function(emitter) {
        self.particleEmitters = self.particleEmitters.filter(function(em) {
            return em != emitter;
        });
    });
