function Blob(options) {
    ...
    this.images = {
        player  : this.constructor.getImage(this.constructor.images.player),
        smaller : this.constructor.getImage(this.constructor.images.smaller),
        bigger  : this.constructor.getImage(this.constructor.images.bigger),
        eyes    : this.constructor.getImage(this.constructor.images.eyes)
    };
}
...
Blob.prototype.squirt = function(target) {
    var direction = this.position.sub(target).normalize().neg(),
        ...
        radius = Math.sqrt(this.mass / (Math.PI * (1 + this.constructor.thrust.mass))),
        squirt = new Blob({
            position : center,
            radius   : this.radius - radius,
            speed    : direction.scale(this.constructor.thrust.speed)
        });

    this.radius = radius;
    this.momentum = this.momentum.sub(squirt.momentum.scale(this.constructor.thrust.factor));
...
Blob.prototype.render = function render(ctx, playerBlob) {
    if (!this._lastRenderTime) this._lastRenderTime = +new Date;
    var img = this.isPlayer ? this.images.player : (this.radius > playerBlob.radius ? this.images.bigger : this.images.smaller);
    var eyes = this.images.eyes;
    ctx.drawImage(img, -this.radius, -this.radius, this.radius * 2, this.radius * 2);
    ctx.rotate(this.rotation + (new Date / 1000));
